...
export default function Gallery() {
  ...
  const activeIdx = Math.max(
    0,
    cats.findIndex((c) => c.key === initial),
  );
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {cats.map((c) => (
            <a
              key={c.key}
              href={`?cat=${c.key}`}
              className={`px-4 py-2 rounded-full border ${c.key === active.key ? "bg-primary text-white border-primary" : "bg-white hover:bg-secondary/60"}`}
            >
              {c.name}
            </a>
          ))}
